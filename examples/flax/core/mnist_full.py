...
class CNNModule(eg.CoreModule):
    ...
    def loss_and_logs(
        ...
        inputs: jnp.ndarray,
        labels: jnp.ndarray,
    ) -> tp.Tuple[jnp.ndarray, tp.Tuple[Logs, M]]:
    ...
    def train_step(
        ...
        (loss, (logs, self)), grads = jax.value_and_grad(
            self.loss_and_logs, has_aux=True
    ...
    def test_step(
        ...
        loss, (logs, self) = self.loss_and_logs(self.params, forward, inputs, labels)
        return logs, self
