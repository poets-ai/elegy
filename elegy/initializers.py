...
class RandomNormal(Initializer):
    ...
    def __call__(self, shape: Shape, dtype: DType) -> np.ndarray:
        ...
        return m + s * jax.random.normal(hooks.next_key(), shape, dtype)
...
class TruncatedNormal(Initializer):
    ...
    def __call__(self, shape: Shape, dtype: DType) -> np.ndarray:
        ...
        unscaled = jax.random.truncated_normal(
            hooks.next_key(), -2.0, 2.0, shape, dtype
        )
        return s * unscaled + m
...
class RandomUniform(Initializer):
    ...
    def __call__(self, shape: Shape, dtype: DType) -> np.ndarray:
        return jax.random.uniform(
            hooks.next_key(), shape, dtype, self.minval, self.maxval
        )
...
class Orthogonal(Initializer):
    ...
    def __call__(self, shape: Shape, dtype: DType) -> np.ndarray:
        ...
        norm_dst = jax.random.normal(hooks.next_key(), matrix_shape, dtype)
