...
class F1Test(TestCase):
    ...
    def test_basic(self):
        ...
        assert np.allclose(
            elegy.metrics.F1()(y_true, y_pred),
        ...
        assert np.allclose(
            ...
            elegy.metrics.F1(threshold=0.3)(
                jnp.asarray(y_true), jnp.asarray(y_pred)
            ),
        )

        assert np.allclose(
            tfa.metrics.F1Score(2, average="micro", threshold=0.3)(y_true, y_pred, sample_weight=sample_weight),
            elegy.metrics.F1(threshold=0.3)(
