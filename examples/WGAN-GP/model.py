...
class WGAN_GP(elegy.Model):
    ...
    def pred_step(self, x, net_states, net_params, states, rng):
        ...
        z = x
        x_fake = self.generator.apply({"parameters": g_params, "states": g_states})(z)[
            0
        ]
    ...
    def train_step(self, x, net_states, net_params, optimizer_states, rng):
        ...
        d_loss, d_variables, d_opt_states, rng, gp = self.discriminator_step_j(
            x, d_params, d_states, g_variables, d_opt_states, rng
        )

        self.i += 1
        # training the generator only every 5 iterations as recommended in the original WGAN paper
        if self.i % 5 == 0:
            g_loss, g_variables, g_opt_states, rng = self.generator_step_j(
                len(x), g_params, g_states, d_variables, g_opt_states, rng
            )
        else:
            g_loss = 0
        ...
        return {"d_loss": d_loss, "g_loss": g_loss, "gp": gp}, elegy.States(
            net_params=(
                g_variables.get("parameters", {}),
                d_variables.get("parameters", {}),
            ),
            net_states=(g_variables.get("states", {}), d_variables.get("states", {})),
    ...
    def discriminator_step(
        self, x_real, d_params, d_states, g_variables, d_opt_states, rng
    ):
        ...
        def d_loss_fn(d_params, d_states, x_real, x_fake, rng):
            ...
            gp = gradient_penalty(
                x_real, x_fake, self.discriminator.apply(d_variables), rng.next()
            )
            loss = loss + gp
            return loss, (d_variables, rng, gp)

        (d_loss, (d_variables, rng, gp)), d_grads = jax.value_and_grad(
    ...
    def generator_step(
        self, batch_size, g_params, g_states, d_variables, g_opt_states, rng
    ):
        ...
        (g_loss, g_variables), g_grads = jax.value_and_grad(g_loss_fn, has_aux=True)(
            g_params, g_states, d_variables, z
        )
