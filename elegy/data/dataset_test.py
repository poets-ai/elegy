...
class DataLoaderTestCase(TestCase):
    def test_basic(self):
        ...
        assert all(batched_y[0] == np.arange(4)) and all(batched_y[1] == np.arange(4)+4) and all(batched_y[2] == np.arange(3)+8)
    ...
    def test_shuffled(self):
        ...
        assert not (all(batched_y0[0] == np.arange(4)) and all(batched_y0[1] == np.arange(4)+4) and all(batched_y0[2] == np.arange(3)+8) )
        assert not (all(batched_y0[0] == batched_y1[0]) and all(batched_y0[1] == batched_y1[1]) and all(batched_y0[2] == batched_y1[2]) )
    ...
    def test_multi_item_ds(self):
        ...
        assert batches0[0].shape==(4,100,200,3) and batches0[1].shape==(4,100,200,3) and batches0[2].shape==(3,100,200,3)
        ...
        class DS1:
            def __len__(self):
                return 11
            def __getitem__(self, i):
                return np.zeros([100,200,3]), np.zeros([100,200,3]), np.zeros([100,200,3])
