...
class ModuleTest(TestCase):
    ...
    def test_set_parameters_shape_check(self):
        ...
        assert np.all(jax.tree_leaves(jax.tree_multimap(lambda x,y: np.allclose(x,y), params0, m.get_parameters())))

        #should not raise an error
        m.set_parameters(params1, check_shapes=True, ignore_on_mismatch=True)
        #linear1.w should not change
        assert np.allclose(m.get_parameters()['linear1']['w'], params0['linear1']['w'])
        #but all others
        assert np.all( jax.tree_multimap(lambda x,y: np.allclose(x,y) if x.shape==y.shape else True, params1, m.get_parameters() ) )
